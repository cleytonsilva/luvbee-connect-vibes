...
-- Date: 2025-11-13
-- Description: Creates georreferenciamento infrastructure with corrected column references
...
BEGIN
  ...
  norm_lat := ROUND(coalesce(NEW.lat, NEW.latitude)::DECIMAL, 6);
  norm_lng := ROUND(coalesce(NEW.lng, NEW.longitude)::DECIMAL, 6);
...
RETURNS TRIGGER AS $$
BEGIN
  IF validate_coordinates(coalesce(NEW.lat, NEW.latitude), coalesce(NEW.lng, NEW.longitude)) THEN
    NEW.location := point(coalesce(NEW.lng, NEW.longitude), coalesce(NEW.lat, NEW.latitude));
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- =============================================
-- 5. VIEW: locations_view (compat layer) - CORRECTED VERSION
-- =============================================

CREATE OR REPLACE VIEW public.locations_view AS
SELECT 
  l.id,
  COALESCE(l.place_id, l.google_place_id) AS place_id,
  l.name,
  l.type,
  l.address,

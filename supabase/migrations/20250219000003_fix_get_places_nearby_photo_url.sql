-- Migration: Fix get_places_nearby photo_url column error
-- Date: 2025-02-19
-- Description: Removes reference to non-existent 'photo_url' column in get_places_nearby function
--              Fixes error: column l.photo_url does not exist
...
RETURNS TABLE (
  ...
  -- photo_url TEXT, -- REMOVED: Column does not exist
  description TEXT,
  phone VARCHAR(20),
  website TEXT,
  rating DECIMAL(3, 2),
...
COMMENT ON FUNCTION get_places_nearby IS 'Returns locations within a radius filtered by user preferences. Fixed to remove invalid photo_url column.';

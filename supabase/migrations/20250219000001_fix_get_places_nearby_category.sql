-- Migration: Fix get_places_nearby column error
-- Date: 2025-02-19
-- Description: Removes reference to non-existent 'category' column in get_places_nearby function
--              Fixes error: column l.category does not exist
...
RETURNS TABLE (
  ...
  name VARCHAR(200),
  -- category VARCHAR(50), -- REMOVED: Column does not exist
  type VARCHAR(50),
  address TEXT,
...
COMMENT ON FUNCTION get_places_nearby IS 'Returns locations within a radius filtered by user preferences. Fixed to remove invalid category column.';

-- Migration: Fix get_places_nearby google_place_data typo
-- Date: 2025-02-19
-- Description: Fixes typo 'google_places_data' -> 'google_place_data' in get_places_nearby function
--              Fixes error: column l.google_places_data does not exist
...
RETURNS TABLE (
  ...
  -- photo_url TEXT, -- REMOVED
  description TEXT,
  -- phone VARCHAR(20), -- REMOVED: Column does not exist
  -- website TEXT, -- REMOVED: Column does not exist
  rating DECIMAL(3, 2),
  price_level INTEGER,
  -- opening_hours JSONB, -- REMOVED: Column does not exist
  google_place_data JSONB, -- FIXED: Typo corrected (was google_places_data)
  owner_id UUID,
  is_active BOOLEAN,
  is_verified BOOLEAN,
  is_curated BOOLEAN,
...
BEGIN
  ...
  SELECT 
    ...
    l.rating,
    l.price_level,
    -- l.opening_hours, -- REMOVED
    l.google_place_data, -- FIXED: Typo corrected
    l.owner_id,
    l.is_active,
    l.is_verified,
    l.is_curated,
...
COMMENT ON FUNCTION get_places_nearby IS 'Returns locations within a radius filtered by user preferences. Fixed typo google_places_data -> google_place_data.';
